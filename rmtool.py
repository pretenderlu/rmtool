...
LEGACY_DEFAULT_FONT_DIR = "/home/root/.local/share/fonts/"
DEFAULT_FONT_DIR = "/home/root/.local/share/rmtool/fonts/"
...
def load_config() -> Dict:
    ...
    if paths["font"] in (
        LEGACY_DEFAULT_FONT_DIR,
        LEGACY_DEFAULT_FONT_DIR.rstrip("/"),
    ):
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def start_app_update_check(self) -> None:
        """Check the public release without delaying the first usable window."""
        if self._app_update_worker is not None:
            return
        worker = Worker(_app_update.check_for_update, APP_VERSION)

        def on_finished(latest_version):
            if sip.isdeleted(self):
                return
            self.connection_widget.set_update_available(latest_version)

        def on_error(exc):
            if sip.isdeleted(self):
                return
            logging.warning("rmtool update check failed: %s", exc)
