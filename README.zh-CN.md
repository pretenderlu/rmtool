...
- **字体管理**：预览并上传多个 TTF/OTF，可在上传时重命名为 `zwzt.ttf`；查看设备用户字体目录顶层文件，按精确文件路径切换系统界面字体，并删除未启用字体。所有上传原文件留在 `/home`，只有当前选中的字体会原子复制到 `/data/rmtool/fonts`，供密码解锁前后的界面共同使用；根分区仅保存一个很小的 Fontconfig 配置。只有旧版根区镜像、rmtool 生成的 Fontconfig、文件元数据以及唯一 `/home` 原字体均精确匹配时，才会提供一键迁移，且无需重新上传字体。系统字体最大 24 MiB，应用后 `/data` 至少保留 32 MiB。界面会按精确设备与固件显示“已实机验证”“待实机验证”或“未实机验证”。上传不会自动切换字体或重启设备，重启由独立确认按钮执行。
