...
- **壁纸管理**：读取设备现有启动、休眠、轮播和关机壁纸预览；按设备尺寸及横竖方向输出，支持留白、裁剪和拉伸，裁剪时可调整水平/垂直偏移；还可把最多 12 个文档缩略图配合可选标题、副标题，在电脑本地生成封面墙壁纸，不会把文档数据发送到云端。
...
| 设备型号 | 平台代号 | 3.27.1.0 正式版（`20260506100933`） | 3.27.3.0 正式版（`20260612085811`） | 3.28.0.162 测试版（`20260629074044`） |
| --- | --- | --- | --- | --- |
| reMarkable Paper Pro | `ferrari` | 官方固件离线验证 | 官方固件离线验证 | **真机验证** |
| reMarkable Paper Pro Move | `chiappa` | 官方固件离线验证 | 官方固件离线验证 | 官方固件离线验证 |
| reMarkable Paper Pure | `tatsu` | 暂不支持 | 官方固件离线验证 | 暂不支持 |
| reMarkable 1 | `rm1` | 暂不支持 | 官方固件离线验证 | 暂不支持 |
| reMarkable 2 | `rm2` | 暂不支持 | 官方固件离线验证 | 暂不支持 |
...
- **无法启用点击翻页**：先点击“检测状态”。设备型号、固件、架构和原始 xochitl 哈希必须精确命中上表中的一项；xochitl 或载荷被修改，或已存在其他 Xovi 持久化配置时，也会阻止部署。
