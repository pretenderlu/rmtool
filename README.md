...
当前发布版为 `v1.16.11`。3.28.0.172 正式版的固件专用资源已覆盖 Paper Pro、Paper Pro Move、Paper Pure、reMarkable 1 和 reMarkable 2，并已发布至对应的 GitHub Release 与腾讯云 COS 镜像；所有目标均完成官方固件离线验证，真机验证范围以各功能支持矩阵中的标注为准。
...
- **壁纸管理**：读取设备现有启动、休眠、轮播和关机壁纸预览；当前界面只按所选设备的原生分辨率生成竖屏壁纸，支持留白、裁剪和拉伸，裁剪时可调整水平/垂直偏移；还可把选中的文档缩略图配合可选文案，在电脑本地排版为封面墙壁纸，不会把文档数据发送到云端。休眠壁纸可选择保存在用户分区，避免占用根分区空间。
...
- **微信读书**：Paper Pro 与 Paper Pro Move 的 3.28 系列固件可直接从[腾讯微信读书官方 CDN](https://cdn.weread.qq.com/app/remarkable/remarkable-weread-v1.0.0-universal-release.zip)在线安装或修复 RemarkableWeRead v1.0.0，并安装与当前固件精确匹配的设备启动入口。官方 43 MB ZIP 不会打进 rmtool，也不会镜像到 rmtool 的 GitHub Release 或腾讯云 COS；rmtool 会先校验外层官方 ZIP，只提取指定的内层安装包并再次校验，然后传输到设备执行未经修改的上游安装脚本。重装会保留登录、下载和阅读数据。启动入口提供普通、彩色快刷与黑白快刷，后两种模式支持独立的定期完整刷新设置；入口安装后需手动重启设备。rmtool 仅为 3.28 系列固件提供此项支持；3.27 及更早固件请参考微信读书官方提供的安装说明。
- **字体管理**：预览并上传多个 TTF/OTF，可在上传时重命名为 `zwzt.ttf`；查看 `/home/root/.local/share/rmtool/fonts` 中的非活动字体，按精确文件路径切换系统界面字体，并删除未启用字体。可按添加顺序把多个自定义字体加入 EPUB 阅读菜单，不再限制为三个，删除后自动前移。上传字体在用户明确选择前不会进入 Fontconfig 默认扫描范围。字体管理提供“为锁屏密码界面保留字体（写入 /data）”选项，默认开启：选中的系统字体会原子复制到 `/data/rmtool/fonts`，供密码解锁前后的界面共同使用，并按替换完成后的实际空间判断，应用后 `/data` 至少保留 24 MiB；关闭后则只在 `/home` 中使用系统字体，不触发 `/data` 空间检查，并会清理 rmtool 管理的旧镜像，但解锁前界面可能仍使用原生字体。已有自定义字体目录继续受支持，不会被自动搬移。只有旧版根区镜像、rmtool 生成的 Fontconfig、文件元数据以及唯一 `/home` 原字体均精确匹配时，才会提供一键迁移，且遵循当前锁屏字体选项。界面会按精确设备与固件显示“已实机验证”“待实机验证”或“未实机验证”。上传不会自动切换字体或重启设备，重启由独立确认按钮执行。
- **时间管理**：同步电脑时间、查看系统时间/硬件时钟/时区，或设置为 `Asia/Shanghai`。
- **设备控制**：重启设备、开启 Wi-Fi SSH，以及为具有 `rm_frontlight` 前光接口的设备提升亮度并安装持久化服务。
- **屏幕预览**：五款受支持设备均可在独立页面动态预览当前画面，支持独立启停、手动刷新和 PNG 另存为；连续预览会根据设备读取速度自动更新，不会在设备资料库创建文件，设备端临时缓冲会在读取后清理。
- **阅读增强**：在精确支持的 Paper Pro 与 Paper Pro Move 3.27/3.28 固件上，增加一个原生设置页，统一控制点击翻页、快刷模式授权和强制刷新；每本 PDF/EPUB 还可在阅读菜单中保存独立刷新模式和刷新策略。阅读菜单提供普通、彩色快刷、黑白快刷三种互斥模式；彩色快刷保留低质量彩色并提升响应，但可能累积彩色残影，黑白快刷牺牲颜色换取更稳定的黑白输出，两者只作用于彩色设备的 PDF/EPUB 阅读页。Paper Pro 与 Paper Pro Move 3.28.0.172 另提供中文划词精确选取；Paper Pro 已完成官方固件离线验证，尚待实机验证。
...
现有稳定汉化借用 xochitl 内置法语槽位，启用期间不能使用法语。程序会先备份原配置和原始 `reMarkable_fr.qm`，并检查当前主字体是否支持简体中文。3.28.0.172 中，Paper Pro、Paper Pro Move 和 Paper Pure 的官方固件已经内置 CJK 字体；reMarkable 1 和 reMarkable 2 仍不含 CJK。老设备缺少字体时，原生简体中文插件会先提示安装随应用提供的约 3.4 MiB 常用字兜底字体，也可以改用本地 TTF/OTF。字体原文件仍由 `/home` 管理，只有当前系统字体的一份校验副本保存在 `/data/rmtool/fonts`；根分区只持久化 `/etc/fonts/conf.d/99-rmtool-ui-font.conf`。因此加密 `/home` 尚未解锁时，密码界面也能使用同一字体，又不会用完整字体占满根分区。EPUB 字体则始终只保存在 `/home`，不触发系统字体的 `/data` 空间限制。启用汉化、应用或修复字体、还原原始界面后，程序会关闭 SSH，且**不会自动重启设备**。启用中文后请手动重启，再进入“设置 > 语言”选择“法语”，中文界面才会正式显示；还原操作只需按提示重启。
...
Move 正式版包已通过实机安装、语言切换、重启和停用测试，并继续精确匹配官方 xochitl 哈希。3.28.0.172 的 Move 固件已内置 CJK 字体；rmtool 仍会在部署前确认当前系统字体支持简体中文。RM1/RM2 缺少字形时，会先提示使用随应用提供的小体积兜底字体，并继续执行系统字体空间检查。
...
阅读增强是 rmtool 当前唯一的阅读插件：使用一个固件精确包，并在设备原生设置中提供总开关、点击翻页、快刷模式授权和强制刷新的独立设置。开启快刷模式授权后，每本 PDF/EPUB 的阅读菜单提供普通、彩色快刷和黑白快刷三种互斥模式。彩色快刷使用已验证的 Animation 刷新模式，保留低质量彩色并提升响应，但可能累积彩色残影；黑白快刷使用 Mono 刷新模式，牺牲颜色换取更稳定的黑白输出。两种快刷都只作用于彩色设备的 PDF/EPUB 阅读页。Paper Pro 与 Paper Pro Move 3.28.0.172 另提供中文划词精确选取，让荧光笔按中文字符边界截断选区，同时保留英文单词扩展和其他原生手势；Paper Pro 已完成官方固件离线验证，尚待实机验证。强制刷新可为该书选择每 5/10/15/20/25/30 次翻页或按章节刷新。同一菜单新增“目录”直达入口：reMarkable 系统本身已经提供目录数据、层级、跳转和原生目录页，rmtool 不重新解析或生成目录，只把原本位于更深层级菜单中的系统目录入口提到外面。全局开关负责授权，单书模式不能绕过全局关闭；关闭全局功能不会删除已经保存的单书状态。rmtool 会同时匹配硬件平台、CPU 架构、内部固件版本和原始 `/usr/bin/xochitl` SHA-256；其他固件或被修改的构建不会通过猜测强行安装。旧版点击翻页和快速黑白包只作为安全迁移、清理的兼容性输入，不再作为独立功能安装。

本次新增的动态 EPUB 字体菜单、中文划词精确选取和用户分区休眠壁纸参考了 [bbq191/rm-tweak](https://github.com/bbq191/rm-tweak) 的实现思路，并沿用 rmtool 自己的固件精确匹配、资源校验和回滚机制重新实现。具体来源与许可证见 [NOTICE.md](NOTICE.md)。
...
安装并手动重启设备后，先在“设置 > 阅读增强”中开启总开关和需要授权的功能，再到每本 PDF/EPUB 的阅读菜单中独立选择模式。单书状态按文档 ID 保存并可跨重启保留。点击翻页会保留原生滑动、手写笔、菜单、缩放、选区和文档链接；彩色快刷和黑白快刷只作用于彩色设备的 PDF/EPUB 阅读，强制刷新不会出现在笔记本、插入笔记页或非阅读页面中。
...
共享插件损坏时，可进入**工具箱 > 旧版插件迁移/清理 > 检测恢复状态**，或使用“检测全部插件”。检测只读；“修复并重装”要求能确认 rmtool 安装归属且固件精确受支持。当前覆盖保守：启用 sidecar（包括拼音输入法）或 AppLoad/KOReader 外部程序、旧 `/home` 布局、被挂载遮盖的路径，以及未知归属、混合运行时、不安全路径、不支持的固件，均会阻止自动恢复。对支持的组合，确认后先校验全部替换资源包，保留原安装备份，再重建全部已启用的共享插件，保留启用/停用状态及外部设置、字体和书籍。不会绕过 SHA 校验；下载失败可使用已验证的手动加载流程。交换安装前会开启临时紧急停用保护，失败时保留；只有修复后严格校验全部通过，才清除本次新增的保护，原有紧急停用标记保持不变。旧 `startup.pending` 仅留在隔离的原安装中，不复制到新安装。结果会显示备份路径和保护状态；成功后按提示处理，再手动重启。若原有紧急停用仍保留，请重新连接，在“原生简体中文”中检测状态，通过校验后再使用“清除紧急停用”。修复失败或结果不确定时，不要重启或解除保护：恢复旧程序不代表旧程序已通过校验，应保留备份并先诊断。严格的固件迁移与历史 Vellum 清理仍是独立操作。若恢复出厂或中断安装只留下固定路径中的残缺共享 XOVI，检测结果会单独显示“可清理不完整状态”；只有核心文件指纹、root 权限、父路径无符号链接、挂载边界和外部启动配置均可确认时才会启用该危险操作。它只移除 rmtool 共享基座、受校验的 drop-in 和紧急标记，不删除微信读书、KOReader、字体、书籍或其他 `/data/rmtool` 内容。该功能发布前未在真实设备执行删除测试，清理前请确保设备具备官方恢复路径；完成后需手动重启并重新安装。
...
所有可卸载的 rmtool 插件现在都区分“停用”和“卸载”：停用会保留已验证的插件文件，卸载会在再次确认后删除该插件及其受控资源，并保留仍被其他插件使用的共享运行时。原生中文卸载前会把当前语言安全切回英文；微信读书的“卸载启动入口”只删除 rmtool 注册的入口、图标和启动插件，不删除官方 App、登录信息、下载内容或阅读数据。KOReader 的卸载会保留 rmtool 白名单用户数据；“彻底清理旧版残留”仍是单独的不可逆操作。未知、被修改、混合或正在运行的安装不会被自动删除。完成后 SSH 会话关闭，需按提示手动重启设备。
