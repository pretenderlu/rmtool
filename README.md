...
- **文档中心**：新增 PDF/EPUB 文档上传流程，可直接从桌面选取文件并推送到设备；同时提供元数据视图、缩略图预览以及导出（优先生成 PDF）功能。
...
3. 首次连接时在界面左上角新建或选择设备条目，填写地址与 root 密码即可。勾选“记住密码”后凭证将保存到系统 keyring。连接成功后即可访问全部功能，无需手动创建 `fonts`、`wallpaper`、`documents` 目录。
...
- **无法导出文档？** 工具会优先下载设备生成的 PDF（含 ZIP 导出的 PDF），若缺失则会基于缩略图生成预览版 PDF，并保留原始资源供后续深度处理。
