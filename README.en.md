...
- **Firmware management**: Currently supports Paper Pro and Paper Pro Move. It can read A/B partition roles and versions, retrieve the official firmware inventory, download and install a verified official image to the standby partition, or switch to an inspected standby partition. After the new firmware boots, rmtool can restore the previous rmtool plugin set when exact packages are available. Follow the in-app restart prompt after installation, switching, or plugin restoration.
- **AppLoad and KOReader**: On exact supported production firmware, install AppLoad and KOReader directly from their official GitHub Releases without Vellum. rmtool verifies the pinned filename, size, and SHA-256, accepts the same official ZIP files for offline import, and never restarts the device automatically. A previous Vellum/AppLoad KOReader directory is fully backed up before rmtool migrates its settings, history, statistics, screenshots, and other allowlisted user data into a clean official install; unrecognized program files are not mixed into the new version. Firmware 3.28 beta builds are intentionally unsupported; stable 3.28.0.172 is also unavailable pending upstream AppLoad support. The existing KOReader library manager can then search folders, transfer books, create folders, and delete entries without leaving the detected library root.
- **WeRead**: Paper Pro and Paper Pro Move on the 3.28 firmware series can install or repair RemarkableWeRead v1.0.0 directly from [Tencent WeRead's official CDN](https://cdn.weread.qq.com/app/remarkable/remarkable-weread-v1.0.0-universal-release.zip) and add an exact-firmware device launcher. The official 43 MB ZIP is neither bundled in rmtool nor mirrored to rmtool GitHub Releases or Tencent COS. rmtool verifies the outer official ZIP, extracts only the expected inner installer archive, verifies it again, and transfers it to the device to run the unchanged upstream installer. Reinstallation preserves login, downloads, and reading data. The launcher offers normal, fast-color, and fast-monochrome modes; each accelerated mode has its own periodic full-refresh setting and launcher changes require a manual device restart. rmtool provides this integration only for 3.28 firmware; for 3.27 and earlier releases, follow the installation instructions provided by WeRead.
- **Font manager**: Preview and upload multiple TTF/OTF fonts, optionally rename an upload to `zwzt.ttf`, inspect the inactive repository at `/home/root/.local/share/rmtool/fonts`, switch the exact font file used by the system UI, and delete inactive fonts. Multiple custom fonts can be added to the EPUB reader menu in insertion order, without the old three-font limit; later entries move up automatically after a removal. Uploads stay outside Fontconfig's default scan paths until explicitly selected. The font manager provides a “Keep the font for the lock-screen passcode screen (write to /data)” option, enabled by default: the selected system font is atomically mirrored to `/data/rmtool/fonts` for the pre-unlock UI, with a post-swap reserve of at least 24 MiB. When disabled, the system font remains under `/home`, skips `/data` capacity checks, and clears rmtool's old mirror; the pre-unlock screen may continue using the stock font. Root stores only a small, persistent Fontconfig file. Existing custom font directories remain supported and are not moved automatically. A one-click migration is offered only when the old root mirror, rmtool-generated Fontconfig files, metadata, and one `/home` original match the exact legacy layout; no font is re-uploaded, and migration follows the same option. The UI labels exact device/firmware identities as device verified, pending validation, or unverified. Uploading does not change the active font or reboot the device; restart is a separate confirmed action.
...
- **Screen preview**: All five supported devices have a dedicated adaptive live-preview page with separate start and stop controls, manual refresh, and PNG Save As. It updates as quickly as serialized device reads allow, creates no library document, and removes its temporary device-side buffer after transfer.
- **Reading enhancements**: On exact supported Paper Pro and Paper Pro Move 3.27/3.28 builds, add one native Settings page for global tap-to-turn, fast-mode authorization, and forced-refresh controls. Each PDF/EPUB reading menu then offers mutually exclusive normal, color-fast, and monochrome-fast modes plus the per-book refresh strategy. Color-fast retains lower-quality color with a more responsive refresh and may accumulate color ghosting; monochrome-fast trades color for steadier monochrome output. Both modes affect only PDF/EPUB reading on color devices. Paper Pro and Paper Pro Move 3.28.0.172 additionally provide precise Chinese/CJK highlight selection; Paper Pro is offline verified and still awaits real-device verification.
...
Open the KOReader page, connect the device, and click **Check Status**. Installation is enabled only when the model, internal firmware version, architecture, and stock xochitl hash exactly match a supported production-firmware entry; stable 3.28.0.172 is not currently supported. Install AppLoad first, then KOReader, and restart the device manually after rmtool closes SSH. If legacy KOReader files are detected, choose either **Migrate and Install KOReader** or **Permanently Remove Legacy Files**. Migration keeps the untouched old directory at `/home/root/.local/share/rmtool/koreader-legacy-backup`; permanent removal deletes the fixed legacy KOReader application directory and everything inside it, creates no backup, and requires a separate confirmation before a clean installation. Online installation downloads AppLoad from [asivery/rm-appload Releases](https://github.com/asivery/rm-appload/releases) and KOReader from [koreader/koreader Releases](https://github.com/koreader/koreader/releases); these two application archives are not bundled in rmtool, copied to Tencent COS, or served from the rmtool repository. For an unreliable connection, download the exact official ZIP yourself and choose **Load Local Official Package**.
...
The established localization path reuses xochitl's built-in French language slot, so French is unavailable while Chinese is enabled. rmtool first backs up the original configuration and `reMarkable_fr.qm`, then checks whether the current primary font supports Simplified Chinese. On stable 3.28.0.172, Paper Pro, Paper Pro Move, and Paper Pure include a stock CJK font, while reMarkable 1 and reMarkable 2 do not. When an older device lacks coverage, the independent Chinese plugin offers a confirmed bundled common-character fallback of about 3.4 MiB, or a local TTF/OTF file. The selected UI font remains managed under `/home`; one verified active copy is stored under `/data/rmtool/fonts`, while only `/etc/fonts/conf.d/99-rmtool-ui-font.conf` is persisted on the root filesystem. This keeps the same font available on the passcode screen before encrypted `/home` is unlocked without consuming root space with a full font. EPUB fonts remain under `/home` and do not use the system-font `/data` space check. After enabling localization, applying or repairing fonts, or restoring the original UI, rmtool closes SSH and **does not restart the device automatically**. After enabling Chinese, restart the device manually, then open Settings > Language and select French to activate the Chinese UI. Restoring the original UI only requires the prompted restart.
...
The Move stable package has passed real-device installation, language switching, reboot, and removal tests. It remains gated to the exact official xochitl hash. Its 3.28.0.172 stock firmware includes CJK coverage; on RM1/RM2, rmtool offers the small bundled fallback only after detecting that the active system font lacks Chinese coverage.
...
Reading enhancements remain one rmtool plugin and one exact-firmware package. The native device Settings page provides a master switch plus independent global settings for tap-to-turn, fast-mode authorization, and forced refresh. After fast-mode authorization is enabled, each PDF/EPUB reading menu exposes mutually exclusive normal, color-fast, and monochrome-fast modes. Color-fast uses the validated Animation screen mode, retaining lower-quality color with a more responsive refresh while potentially accumulating color ghosting; monochrome-fast uses Mono and intentionally trades color for steadier monochrome output. Both modes affect only PDF/EPUB reading on color devices. The offline-verified Paper Pro and Paper Pro Move 3.28.0.172 builds additionally provide precise Chinese/CJK highlight selection, snapping highlights to CJK character boundaries while preserving stock English word expansion and other gestures; Paper Pro still awaits real-device verification. Forced refresh can use a 5/10/15/20/25/30-page interval or chapter boundaries for that document. The same menu also provides a direct **Table of contents** entry: reMarkable already supplies the TOC data, hierarchy, navigation, and native screen, while rmtool only exposes that existing screen outside the deeper stock menu. Global settings always win over per-book state, while disabling the global feature preserves the saved book preferences. rmtool requires the hardware platform, CPU architecture, internal firmware version, and stock `/usr/bin/xochitl` SHA-256 to match; other or modified builds are rejected rather than guessed. The older tap-to-turn and fast-monochrome packages remain private compatibility inputs for safe migration and cleanup only.

The new dynamic EPUB font menu, precise Chinese/CJK highlight selection, and user-partition sleep wallpaper are based on approaches from [bbq191/rm-tweak](https://github.com/bbq191/rm-tweak), reimplemented within rmtool's exact-firmware checks, resource verification, and rollback model. See [NOTICE.md](NOTICE.md) for attribution and license details.
