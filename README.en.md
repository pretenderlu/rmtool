...
- **Device screenshots**: Enable the firmware's native screenshot shortcut, which continues to save captures to the device library. On Paper Pro Move, rmtool can also capture the current screen directly as a PNG on the computer without creating a library PDF; its temporary device-side buffer is removed after transfer.
